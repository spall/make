...
// result = x - y - z                                                                                
int timespec_subtract_3(struct timespec *result, struct timespec *x, struct timespec *y, struct time\
spec *z) {
  ...
  if (1 == timespec_subtract(tmp, x, y)) {
    fprintf(stderr, "Negative time: %lld.%ld - %lld.%ld\n", (long long)x->tv_sec, x->tv_nsec, (long \
long)y->tv_sec, y->tv_nsec);
    exit(EXIT_FAILURE);
  }

  if (1 == timespec_subtract(result, tmp, z)) {
    fprintf(stderr, "Negative time: %lld.%ld - %lld.%ld\n", (long long)tmp->tv_sec, tmp->tv_nsec, (l\
ong long)z->tv_sec, z->tv_nsec);
    exit(EXIT_FAILURE);
  }
